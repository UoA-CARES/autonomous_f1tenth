    ...
    for step_counter in range(max_steps_training):
        ...
        if step_counter < max_steps_exploration:
            ...
            action_env = np.random.uniform(
                env.MIN_ACTIONS, env.MAX_ACTIONS, env.ACTION_NUM
            )
        ...
        if done or truncated:
            ...
            if evaluate:
                evaluate = False
                env.get_logger().info(
                    f"*************--Begin Evaluation Loop--*************"
                )
                off_policy_evaluate(
                    env, agent, number_eval_episodes, record, step_counter
                )
                env.get_logger().info(
                    f"*************--End Evaluation Loop--*************"
                )
    ...
    for episode_num in range(eval_episodes):
        ...
        while not done and not truncated:
            ...
            with open("network_output.csv", "a") as f:
                f.write(
                    f"{episode_num},{episode_timesteps},{action[0]:.4f},{action[1]:.4f}\n"
                )
            next_state, reward, done, truncated, step_info = env.step(
                action_env, is_training=False
            )
            state = next_state
...
def multi_off_policy_train(env, env2, agent, memory, record, algorithm_config):
    ...
    for step_counter in range(max_steps_training):
        ...
        if step_counter < max_steps_exploration:
            # env.get_logger().info(f'Exploration Step #{step_counter} out of {max_steps_exploration}')
            action_env1 = np.random.uniform(
                env.MIN_ACTIONS, env.MAX_ACTIONS, env.ACTION_NUM
            )
            action1 = hlp.normalize(action_env1, env.MAX_ACTIONS, env.MIN_ACTIONS)
            action_env2 = np.random.uniform(
                env2.MIN_ACTIONS, env2.MAX_ACTIONS, env2.ACTION_NUM
            )
        ...
        if done1 or truncated1:
            ...
            if evaluate:
                ...
                multi_off_policy_evaluate(
                    env, env2, agent, number_eval_episodes, record, step_counter
                )
                env.get_logger().info(
                    f"*************--End Evaluation Loop--*************"
                )
...
def multi_off_policy_evaluate(
    env, env2, agent, eval_episodes, record=None, steps_counter=0
):
    ...
    for episode_num in range(eval_episodes):
        ...
        while not done1 and not truncated1 and not done2 and not truncated2:
            ...
            with open("network_output.csv", "a") as f:
                f.write(
                    f"{episode_num},{episode_timesteps},{action1[0]:.4f},{action1[1]:.4f}\n"
                )
            next_state1, reward1, done1, truncated1, step_info1 = env.step(action_env1)
            state1 = next_state1
