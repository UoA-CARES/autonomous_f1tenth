...
def avg_lidar(lidar: LaserScan, num_points: int):
    ranges = np.nan_to_num(lidar.ranges, nan=float(10), posinf=float(10), neginf=float(10))
    ranges = ranges[1:]                                           
    new_range = []
...
def avg_lidar_w_consensus(lidar:LaserScan, num_points:int):
    ...
    ranges = np.nan_to_num(lidar.ranges, nan=float(-5), posinf=float(-5), neginf=float(-5))
