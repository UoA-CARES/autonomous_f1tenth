...
def median_lidar(lidar: LaserScan, num_points: int):
    ranges = lidar.ranges
    ranges = np.nan_to_num(ranges, nan=float(
        10), posinf=float(10), neginf=float(10))
    ranges = ranges[1:]
...
def avg_lidar_w_consensus(lidar: LaserScan, num_points: int):
    ...
    ranges = lidar.ranges
    ranges = np.nan_to_num(ranges, nan=float(-5),
                           posinf=float(-5), neginf=float(-5))
    # ranges = ranges[1:]
    ...
    for i in range(num_points):
        ...
        # sector == -5 returns a boolean array, true being any -5 occurance. sum just count trues. chatgpt black magic right here
...
def process_lidar_med_filt(lidar: LaserScan, window_size: int, nan_to=-5):
    ...
    ranges = np.nan_to_num(ranges, posinf=nan_to,
                           nan=nan_to, neginf=nan_to).tolist()
    processed_ranges = scipy.ndimage.median_filter(
        ranges, window_size, mode='nearest').tolist()
...
def process_ae_lidar(lidar: LaserScan, ae_model, is_latent_only=True):
    ...
    range_tensor = torch.tensor(
        range_list, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
...
def create_lidar_msg(lidar: LaserScan, num_points: int, lidar_range: list):
    ...
    scan.angle_increment = lidar.angle_max*2 / \
        (num_points-1)  # 240/num_points * (3.142 / 180)
...
def forward_reduce_lidar(lidar: LaserScan):
    ...
    idx = np.round(np.linspace(idx_cut, len(ranges) -
                   (1+idx_cut), num_outputs)).astype(int)
    new_range = []
...
def get_all_goals_and_waypoints_in_multi_tracks(track_name):
    ...
    if track_name == 'multi_track_full':
        ...
        budapest_gp = [[x + 200, y]
                       for x, y in goal_positions['budapest_track']]
        hockenheim_gp = [[x + 300, y]
                         for x, y in goal_positions['hockenheim_track']]
        melbourne_gp = [[x + 500, y]
                        for x, y in goal_positions['melbourne_track']]
        saopaolo_gp = [[x + 600, y]
                       for x, y in goal_positions['saopaolo_track']]
        shanghai_gp = [[x + 750, y]
                       for x, y in goal_positions['shanghai_track']]
        ...
        budapest_wp = [(x + 200, y, yaw, index)
                       for x, y, yaw, index in waypoints['budapest_track']]
        hockenheim_wp = [(x + 300, y, yaw, index)
                         for x, y, yaw, index in waypoints['hockenheim_track']]
        melbourne_wp = [(x + 500, y, yaw, index)
                        for x, y, yaw, index in waypoints['melbourne_track']]
        saopaolo_wp = [(x + 600, y, yaw, index)
                       for x, y, yaw, index in waypoints['saopaolo_track']]
        shanghai_wp = [(x + 750, y, yaw, index)
                       for x, y, yaw, index in waypoints['shanghai_track']]
        ...
        saopaolo_wp = [(x + 100, y, yaw, index)
                       for x, y, yaw, index in waypoints['saopaolo_track']]
        shanghai_wp = [(x + 250, y, yaw, index)
                       for x, y, yaw, index in waypoints['shanghai_track']]
        ...
        narrow_track_04_wp = [(x + 187, y, yaw, index)
