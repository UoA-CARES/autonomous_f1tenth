...
class F1TenthReset(Node):
    ...
    def __init__(self, env_name):
        ...
        self.srv = self.create_service(
            Reset, name, callback=self.service_callback, callback_group=srv_cb_group
        )

        set_pose_cb_group = MutuallyExclusiveCallbackGroup()
        self.set_pose_client = self.create_client(
            SetEntityPose, f"world/empty/set_pose", callback_group=set_pose_cb_group
        )
    ...
    def service_callback(self, request, response):
        ...
        car_req = self.create_request(
            request.car_name, x=request.cx, y=request.cy, z=0, yaw=request.cyaw
        )
...
def main():
    ...
    services.spawn(
        sdf_filename=f"{pkg_environments}/sdf/goal.sdf", pose=[1, 1, 1], name="goal"
    )
