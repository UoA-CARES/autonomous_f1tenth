...
class TwoCarEnvironment(F1tenthEnvironment):
    ...
    def __init__(
        ...
        #####################################################################################################################
        # Reward params ----------------------------------------------
        self.REWARD_MODIFIERS: List[
            Tuple[Literal["turn", "wall_proximity", "racing"], float]
        ] = [("turn", 0.3), ("wall_proximity", 0.7), ("racing", 1)]
        ...
        self.STATUS_LOCK_PUB = self.create_publisher(String, "/status_lock", 10)

        self.STATUS_LOCK_SUB = self.create_subscription(
            String, "/status_lock", self.status_lock_callback, 10
        )
        ...
        if self.NAME == "f2tenth":
            while "respawn" not in self.STATUS:
                rclpy.spin_until_future_complete(
                    self, self.STATUS_OBSERVATION_FUTURE, timeout_sec=10
                )
    ...
    def car_spawn(self):
        ...
        if width > 200:
            if order == 1:
                car_2_x, car_2_y = lateral_translation(
                    (car_x, car_y), car_yaw, translation1
                )
                car_x, car_y = lateral_translation(
                    (car_x, car_y), car_yaw, translation2
                )
            else:
                car_2_x, car_2_y = lateral_translation(
                    (car_x, car_y), car_yaw, translation2
        ...
        goal_x, goal_y, _, _ = self.CURR_WAYPOINTS[
            (
                self.SPAWN_INDEX + 3
                if self.SPAWN_INDEX + 3 < len(self.CURR_WAYPOINTS)
                else 0
            )
        ]
        self.GOAL_POS = [x, y]
    ...
    def is_truncated(self):
        return self.PROGRESS_NOT_MET_COUNTER >= 5 or self.STEP_COUNTER >= self.MAX_STEPS
        ...
        match self.LIDAR_PROCESSING:
            case "pretrained_ae":
                processed_lidar_range = process_ae_lidar(
                    lidar, self.AE_LIDAR, is_latent_only=True
                )
                visualized_range = reconstruct_ae_latent(
                    lidar, self.AE_LIDAR, processed_lidar_range
                )
    ...
    def compute_reward(self, state, next_state, raw_lidar_range):
        ...
        progression1 = self.CURR_TRACK_MODEL.get_distance_along_track(
            self.LAST_POS1, odom1[:2]
        )
        progression2 = self.CURR_TRACK_MODEL.get_distance_along_track(
            self.LAST_POS2, odom2[:2]
        )
        ...
        if self.NAME == "f1tenth":
            base_reward, base_reward_info = self.calculate_total_progress_reward(
                self.EP_PROGRESS1, progression1, next_state, raw_lidar_range
            )
        else:
            base_reward, base_reward_info = self.calculate_total_progress_reward(
                self.EP_PROGRESS2, progression2, next_state, raw_lidar_range
            )
        reward += base_reward
        ...
        for modifier_type, weight in self.REWARD_MODIFIERS:
            match modifier_type:
                ...
                case "turn":
                    ...
                    steering_angle2 = twist_to_ackermann(
                        next_state[7], next_state[6], L=0.325
                    )
                ...
                case "racing":
                    if self.NAME == "f1tenth":
                        if self.EP_PROGRESS1 == self.EP_PROGRESS2:
                            ...
                            modifier = (self.EP_PROGRESS1 - self.EP_PROGRESS2) / abs(
                                self.EP_PROGRESS1 - self.EP_PROGRESS2
                            )
                    else:
                        if self.EP_PROGRESS1 == self.EP_PROGRESS2:
                            modifier = 0
                        else:
                            modifier = (self.EP_PROGRESS2 - self.EP_PROGRESS1) / abs(
                                self.EP_PROGRESS2 - self.EP_PROGRESS1
                            )
    ...
    def calculate_total_progress_reward(
        self, total_progression, step_progression, next_state, raw_range
    ):
        reward = 0
