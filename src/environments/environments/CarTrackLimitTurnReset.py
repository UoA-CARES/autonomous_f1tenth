...
class CarTrackLimitTurnReset(Node):
    def __init__(self):
        super().__init__('car_track_limit_turn_reset')

        srv_cb_group = MutuallyExclusiveCallbackGroup()
        self.srv = self.create_service(Reset, 'car_track_limit_turn_reset', callback=self.service_callback, callback_group=srv_cb_group)
...
def main():
    ...
    reset_service = CarTrackLimitTurnReset()
    pkg_environments = get_package_share_directory('environments')
