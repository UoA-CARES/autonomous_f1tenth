...
class CarTrackReset(Node):
    def __init__(self):
        super().__init__('car_track_progressive_goal_reset')

        srv_cb_group = MutuallyExclusiveCallbackGroup()
        self.srv = self.create_service(Reset, 'car_track_progressive_goal_reset', callback=self.service_callback, callback_group=srv_cb_group)
