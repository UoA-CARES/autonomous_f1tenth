...
class TwoCarReset(Node):
    def __init__(self):
        super().__init__('two_car_reset')

        srv_cb_group = MutuallyExclusiveCallbackGroup()
        self.srv = self.create_service(Reset, 'two_car_reset', callback=self.service_callback, callback_group=srv_cb_group)
