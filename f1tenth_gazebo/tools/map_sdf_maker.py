...
def create_track_element(track_name: str, offset_x: int):
    ...
    uri_visual.text = f"model://f1tenth_gazebo/meshes/{track_name}.stl"
    ...
    uri_collision.text = f"model://f1tenth_gazebo/meshes/{track_name}.stl"
...
if __name__ == "__main__":
    ...
    if IS_CREATING_INDIVIDUAL_TRACKS:
        ...
        for track_name in TRACK_NAMES:
            for width in WIDTHS:
                current_track_name = f"{track_name}_{str(width)}"
                element = create_track_element(
                    current_track_name, offset_x=i * 30
                )  # each track offset by 30m
                root.find("world").append(element)
                i += 1
