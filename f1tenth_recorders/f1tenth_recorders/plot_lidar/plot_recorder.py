...
DEFAULT_RECORD_DIR = os.environ.get("LIDAR_PLOT_RECORD_DIR", os.path.dirname(__file__))
VEL_RECORD = os.environ.get(
    "VEL_RECORD_FILE",
    os.path.join(DEFAULT_RECORD_DIR, "record_sim_2025-07-23 15:52:16.txt"),
)
LIDAR_RECORD = os.environ.get(
    "LIDAR_RECORD_FILE",
    os.path.join(DEFAULT_RECORD_DIR, "record_lidar_2025-07-23 15:52:16.txt"),
)
...
def read_lidar_data(lidar_file_path):
    ...
    data = None
    with open(lidar_file_path, "r", encoding="utf-8") as lidar_file:
        ...
        for line in lines:
            ...
            if not line:
                ...
                distance = math.sqrt((x - data.odom[0]) ** 2 + (y - data.odom[1]) ** 2)
...
def read_vel_data(vel_file_path):
    ...
    with open(vel_file_path, "r", encoding="utf-8") as vel_file:
        ...
        for line in lines:
            ...
            parts = line.split(",")
            if len(parts) == 3:
                time, linear, angular = (float(part.split("=")[1]) for part in parts)
                data = VelData(time)
...
def plot():
    ...
    slider = Slider(
        ...
        "Index",
        1,
        min(len(lidar_data_list), len(vel_data_list)) - 1,
        valinit=1,
        valstep=1,
    )
...
if __name__ == "__main__":
    ...
    agent.actor_net.load_state_dict(torch.load(ACTOR, map_location=torch.device("cpu")))
    agent.critic_net.load_state_dict(
        torch.load(CRITIC, map_location=torch.device("cpu"))
    )
