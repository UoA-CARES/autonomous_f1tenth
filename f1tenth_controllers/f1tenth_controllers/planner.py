...
import time
import os
from .path_utils import absoluteDistance
import yaml
...
def main():
    ...
    param_node.declare_parameters(
        "", [("alg", "random"), ("map", "random"), ("yaml_path", "random")]
    )
    ...
    if image is None:
        ...
        ret, thresholded_image = cv2.threshold(
            image, threshold_value, max_value, cv2.THRESH_BINARY_INV
        )
        ...
        if 1 == 0:
            ...
            for rho, theta in lines:
                ...
                dist = abs(
                    (y2 - y1) * dot_x - (x2 - x1) * dot_y + x2 * y1 - y2 * x1
        ...
        threshold_value = 127  # Example threshold value
        max_value = 255  # Maximum pixel value after thresholding
        ret, add_car_image = cv2.threshold(
            add_car_image, threshold_value, max_value, cv2.THRESH_BINARY_INV
        )
