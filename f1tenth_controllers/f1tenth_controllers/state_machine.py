...
class StateMachine(Node):
    def __init__(self, startStage):
        ...
        self.odomController = Controller("odom_controller", "f1tenth", 100)
    ...
    def mappingState(self):
        ...
        while absoluteDistance(np.array(self.init_odom), np.array(self.odom)) > 1.2:
            stringToPrint = (
                ...
                + str(self.odom)
                + ", distance: "
                + str(absoluteDistance(np.array(self.init_odom), np.array(self.odom)))
            )
...
def main():
    ...
    param_node.declare_parameters("", [("startStage", "init")])
