...
class PurePursuit:
    ...
    def findGoal(self, location):
        ...
        if (
            lastPointInd < 0
        ):  # If no points within lookahead range, goal1 is closest point
        ...
        if discriminant < 0:
            ...
            goal = np.asarray(
                [goal1[0] + x * np.cos(theta2), goal1[1] + x * np.sin(theta2)]
            )
        else:
            x1 = (-b - np.sqrt(discriminant)) / (2 * a)
            x2 = (-b + np.sqrt(discriminant)) / (2 * a)
            goalx1 = np.asarray(
                [goal1[0] + x1 * (np.cos(theta2)), goal1[1] + x1 * (np.sin(theta2))]
            )
            goalx2 = np.asarray(
                [goal1[0] + x2 * (np.cos(theta2)), goal1[1] + x2 * (np.sin(theta2))]
            )
            if (abs(goal2[0] - goalx1[0]) < abs(goal2[0] - goal1[0])) & (
                abs(goal2[1] - goalx1[1]) < abs(goal2[1] - goal1[1])
            ):
                return goalx1
            else:
                return goalx2
