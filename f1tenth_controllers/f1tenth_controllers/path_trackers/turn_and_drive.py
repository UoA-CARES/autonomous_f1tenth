...
class TurnAndDrive:
    ...
    def __init__(
        self,
        turning_lin_vel=0.2,
        turning_ang_modifier=1,
        straight_lin_vel=0.8,
        angle_diff_tolerance=0.1,
        goal_tolerance=0.2,
        steering_to_neutral_delay=0.5,
    ):
    ...
    def select_action(self, state, goal, nextGoal):
        ...
        if (abs(distance[0]) < self.goal_tolerance) and (
            abs(distance[1] < self.goal_tolerance)
        ):
            goal = nextGoal
            ang = turn_to_goal(location, self_angle, goal)
            distance = goal - location
        ...
        if abs(ang) > 0:
            ...
            if self.turnedLast:
                ...
                self.steering_neutral_timer = threading.Timer(
                    self.steering_to_neutral_delay,
                    self.complete_waiting_for_steering_neutral,
                )
