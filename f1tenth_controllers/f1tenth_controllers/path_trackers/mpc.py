...
class MPC:
    ...
    def newStates(self, lin, x, y, steeringAngle, desAngle, yaw):
        ...
        steeringAngle = (
            steeringAngle
            - self.timeConst ** (-1) * (steeringAngle - desAngle) * self.deltaT
        )
    ...
    def cost(
        ...
        cost = (
            Yarr @ Q @ np.transpose(Yarr) + qsteer * (steeringcurr - steering) ** 2
        )  # (U - Uref)*R*(U-Uref)
        return cost
    ...
    def select_action(self, state, goal, nextGoal):
        ...
        for i in range(self.options):
            ...
            for j in range(self.predictionSteps):
                x, y, yaw, steeringAngle = self.newStates(
                    lin, x, y, steeringAngle, desAngle, yaw
                )
            cost = self.cost(
                xcurr,
                x,
